...
export async function run(ctx) {
  ...
  await withPage(browser, "/", async (page) => {
    ...
    await paletteSearchbox.fill("export");
    expect(
    ...
    expect(
      (await page.getByRole("dialog", { name: "Workspace search", exact: true }).count()) === 0,
      "Running a settings command should close the launcher and open the existing Preferences surface.",
    );
    await page.keyboard.press("Escape");
    await page.keyboard.press("Escape");
    await page.keyboard.press("ControlOrMeta+K");
