...
type WorkspaceFolderImportCopy = {
  ...
  title: string;
  description: string;
  summary: (markdown: number, assets: number) => string;
...
const copies: Record<WorkspaceLanguage, RawWorkspaceFolderImportCopy> = {
  en: {
    ...
    summary: (markdown, assets) =>
      `${markdown} Markdown · ${assets} ${assets === 1 ? "asset" : "assets"}`,
    markdownPaths: "Markdown documents",
    supportPaths: "Preserved bundle assets",
    supportNote: "All non-Markdown files are preserved unchanged in exports. Tabula does not treat them as knowledge documents or execute them.",
  ...
  ko: {
    ...
    summary: (markdown, assets) =>
      `Markdown ${markdown}개 · 번들 자산 ${assets}개`,
    markdownPaths: "Markdown 문서",
    supportPaths: "보존할 번들 자산",
    supportNote: "Markdown이 아닌 모든 파일은 내보내기에 원형 그대로 보존합니다. Tabula는 이를 지식 문서로 취급하거나 실행하지 않습니다.",
  ...
  ja: {
    ...
    summary: (markdown, assets) => `Markdown ${markdown} 件 · バンドル資産 ${assets} 件`,
    markdownPaths: "Markdown 文書",
    supportPaths: "保持するバンドル資産",
    supportNote: "Markdown 以外のすべてのファイルは変更せずにエクスポートへ保持されます。Tabula はそれらを知識文書として扱わず、実行もしません。",
  ...
  zh: {
    ...
    summary: (markdown, assets) => `${markdown} 个 Markdown · ${assets} 个捆绑资源`,
    markdownPaths: "Markdown 文档",
    supportPaths: "保留的捆绑资源",
    supportNote: "所有非 Markdown 文件都会原样保留在导出中。Tabula 不会将其视为知识文档，也不会执行它们。",
  ...
  es: {
    ...
    summary: (markdown, assets) => `${markdown} Markdown · ${assets} recursos`,
    markdownPaths: "Documentos Markdown",
    supportPaths: "Recursos del paquete conservados",
    supportNote: "Todos los archivos que no son Markdown se conservan sin cambios al exportar. Tabula no los trata como documentos de conocimiento ni los ejecuta.",
  ...
  fr: {
    ...
    summary: (markdown, assets) => `${markdown} Markdown · ${assets} ressources`,
    markdownPaths: "Documents Markdown",
    supportPaths: "Ressources du bundle conservées",
    supportNote: "Tous les fichiers non Markdown sont conservés sans modification lors de l’export. Tabula ne les traite pas comme des documents de connaissance et ne les exécute pas.",
  ...
  de: {
    ...
    summary: (markdown, assets) => `${markdown} Markdown · ${assets} Bundle-Dateien`,
    markdownPaths: "Markdown-Dokumente",
    supportPaths: "Erhaltene Bundle-Dateien",
    supportNote: "Alle Nicht-Markdown-Dateien bleiben beim Export unverändert erhalten. Tabula behandelt sie nicht als Wissensdokumente und führt sie nicht aus.",
