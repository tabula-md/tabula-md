...
export const createWorkspaceModelState = ({
  files,
  openFileIds,
  activeFileId,
}: WorkspaceModelState): WorkspaceModelState => {
...
export const renameWorkspaceFile = (
  ...
  fileId: string,
  nextRawTitle: string,
): { state: WorkspaceModelState; result: RenameFileResult } => {
  ...
  return {
    state: {
      ...state,
      files: state.files.map((file) => (file.id === fileId ? { ...file, title: nextTitle } : file)),
    },
    result: { ok: true, title: nextTitle },
  };
};

export const selectWorkspaceFile = (state: WorkspaceModelState, fileId: string): WorkspaceModelState => {
...
export const setWorkspaceActiveFileId = (state: WorkspaceModelState, fileId: string): WorkspaceModelState => {
  if (!fileId) {
    return {
      ...state,
      activeFileId: "",
...
export const closeWorkspaceFile = (
  state: WorkspaceModelState,
  fileId: string,
): { state: WorkspaceModelState; result: CloseFileResult } | undefined => {
...
export const reorderOpenWorkspaceFile = (
  state: WorkspaceModelState,
  sourceFileId: string,
  targetFileId: string,
): WorkspaceModelState => {
...
export const workspaceReducer = (
  ...
  switch (action.type) {
    ...
    case "setActiveFileReadingWidth":
      return updateActiveWorkspaceFile(state, (file) => ({ ...file, readingWidth: action.readingWidth }));
